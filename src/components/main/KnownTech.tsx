...
export const KnownTech = ({ tech }: KnownTechProps) => {
  const years = tech.start_date ? differenceInYears(new Date(), tech.start_date) : 0
  const relativeTime = years > 0 ? `${years} ano${years > 1 ? 's' : ''}` : 'Menos de um ano'
  return (
