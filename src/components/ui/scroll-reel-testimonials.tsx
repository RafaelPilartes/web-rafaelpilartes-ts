 ...
 * Adapted to the site's dark theme (no shadcn tokens).
 * ---------------------------------------------------------------- */
...
const QUOTE_CLASSES =
  'm-0 text-lg font-medium leading-[1.3] tracking-[-0.02em] text-white sm:text-[22px]'
const AUTHOR_CLASSES = 'm-0 text-sm font-medium leading-[1.3] text-white/50'

const FEATURED_SHADOW =
  '0 19.148px 13.404px -3.375px rgba(0,0,0,0.4), 0 60px 42px -4.5px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.1)'
...
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-white/10 bg-gradient-to-b from-white/[0.07] to-white/[0.02] blur-[1px]"
      style={{ width: CELL, height: CELL }}
    />
  )
}

/* Featured portrait tile with brand sheen overlay */
function Featured({ src, alt }: { src: string; alt?: string }) {
  return (
    ...
    >
      ...
      />
      {/* diagonal brand sheen */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 z-[3] blur-[6px] mix-blend-overlay"
        style={{
          background:
            'linear-gradient(220.99deg, rgba(241,48,36,0) 32%, rgba(241,48,36,0.9) 41%, rgba(255,150,140,0.8) 47%, rgba(241,48,36,0.4) 54%, rgba(241,48,36,0) 65%)'
        }}
      />
    </div>
  )
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        'relative flex w-full max-w-[1060px] flex-col items-stretch gap-2.5 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] outline-none focus-visible:ring-2 focus-visible:ring-accent md:min-h-[320px] md:flex-row',
        className
      )}
    >
      {/* Reel section */}
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        <div className="flex flex-col gap-[9px]">
          <svg
            className="block h-12 w-12 text-accent/40"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
        ...
        <div className="mt-6 flex items-center gap-1.5 md:mt-0">
          <button
            ...
            className="grid h-7 w-7 cursor-pointer place-items-center rounded-full border border-white/15 bg-transparent p-0 text-white transition-transform duration-200 hover:enabled:scale-[1.08] active:enabled:scale-[0.94] disabled:cursor-default disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
          >
            <svg
